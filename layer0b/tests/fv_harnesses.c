/* Formal verification harnesses — GCC -fanalyzer targeted */
...
/* H1: exec1 sb=i*4, worst case sb+3 < MAX_STATE */
void h_exec1_state(void){
...
/* H2: exec1_ex state_offset never exceeds MAX_STATE */
void h_exec1ex_state(void){
...
/* H3: ADSR bit-field masking — all indices in [0,31] for any uint16_t */
void h_adsr_mask(uint16_t hi, uint16_t lo){
    int ai=(hi>>10)&0x1F, di=(hi>>5)&0x1F, si=hi&0x1F, ri=(lo>>11)&0x1F;
    if(ai>31||di>31||si>31||ri>31) __builtin_trap();
...
/* H4: BPF filter table guards hi<64 and lo<32 */
void h_filter_guards(uint16_t hi, uint16_t lo){
...
/* H5: hash_event_stream malloc/free balance on all paths */
void h_hash_es_memory(void){
...
void h_canonicalize(void){
    PatchProgram prog; memset(&prog,0,sizeof(prog));
    if(prog.n_instrs<0||prog.n_instrs>MAX_INSTRS) return;
    patch_canonicalize(&prog);
}
/* H7: TempoMap binary search never OOB */
void h_tempo_search(void){
    TempoMap m; memset(&m,0,sizeof(m));
    if(m.n_segs<1||m.n_segs>TEMPO_MAX_PTS) return;
    double beat; tempo_beat_to_seconds(&m,beat);
}
/* H8: voice_compile events never exceed VOICE_MAX_EVENTS */
void h_voice_compile(void){
...
/* H9: SYNC state uses sb and sb+1 only */
void h_sync_state(void){
    PatchProgram prog; prog.n_instrs=2; prog.n_regs=6;
...
/* H10: hash_patch_prog reads code[0..n_instrs-1] only */
void h_hash_patch(void){
    PatchProgram prog; memset(&prog,0,sizeof(prog));
    if(prog.n_instrs<0||prog.n_instrs>MAX_INSTRS) return;
